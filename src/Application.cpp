...
App::App()
    : Battery::Application(0, 0, APPLICATION_NAME)
{
...
bool App::OnStartup()
{
  ...
  if (monitorSize.x / SCREEN_SIZE_MARGIN < DEFAULT_WINDOW_WIDTH
      || monitorSize.y / SCREEN_SIZE_MARGIN < DEFAULT_WINDOW_HEIGHT) {
...
void App::OnUpdate()
{
  ...
  if (GetRuntime() < (lastScreenRefresh + passiveScreenTime) && framecount > 60 && !window.IsFocused()
...
void App::OnRender() { using namespace Battery; }
...
Battery::Application* Battery::CreateApplication() { return new App(); }
...
public:
        ...
        bool OnStartup() override {
                ...
                shader = new ShaderProgram(window.allegroDisplayPointer, "../resource/vertex.glsl",
"../resource/fragment.glsl");
